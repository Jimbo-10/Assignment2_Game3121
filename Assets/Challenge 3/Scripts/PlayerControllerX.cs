...
public class PlayerControllerX : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        floating = inputAction.FindAction("Jump");
        floating.Enable();
    }
    ...
    void Update()
    {
        ...
        float y = floating.ReadValue<float>();
        if (y > 0f && !gameOver)
        {
        ...
        if (pos.y >= maxHeight)
        {
            pos.y = math.clamp(pos.y, minHeight, maxHeight);//new Vector3(transform.position.x, maxHeight, transform.position.z);
